...
def processVotes(MongoInstance, pID,uID):
  # print 'Processing past votes of ' + pID

  votes = MongoInstance.client[pID]['votes'].find({'uID':uID, 'user_flagged': False}).sort('submit_time', 1)
  # votes = MongoInstance.client[pID]['votes'].find( { 'user_flagged': { '$exists': False} } ).sort('submit_time', 1)
  # print votes.count()
  ...
  for vote in votes:
    ...
    for cID in vote_result:
      if MongoInstance.client[pID]['user_scores'].find_one({'cID':cID,'uID':uID}) == None:
        
        MongoInstance.initialize_score_user(pID, uID, cID)

      scoreDoc = MongoInstance.client[pID]['user_scores'].find_one({'cID':cID,'uID':uID})
    ...
    for updated_score in updated_scores:
      ...
      MongoInstance.updateScore_user(pID, uID, cID, mID, new_mu, new_sigma, isTie)

    vID = str(vote['_id'])
    MongoInstance.setVoteFlagged_user(pID, vID)

  # print 'Done processing votes!'


# calculate percentiles, and solve for ranks
def calculatePercentilesAndRanks(MongoInstance, pID, uID):
  ...
  allScoreDocs = MongoInstance.client[pID]['user_scores'].find({'uID':uID})
  ...
  for scoreDoc in allScoreDocs:
    ...
    cID = scoreDoc['cID']
    MongoInstance.updatePercentilesAndRanks_user(pID,uID,cID,new_vote_percentiles,new_tie_percentiles,new_ranks, new_votability)
